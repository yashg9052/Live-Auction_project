...
interface AuctionDetail {
  ...
  current_highest_bid_time: string | null;
  images: string[];
  category: string;
...
export default function AuctionDetailPage({
  ...
  const { isLoggedIn, user, setUser } = useAuctionData();
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // ✅ Normalize to string to avoid type mismatch with user._id
        setHighestBidderId(
          data.current_highest_bidder_id != null
            ? String(data.current_highest_bidder_id)
            : null,
        );
        setBidValue((data.current_highest_bid ?? 0) + 500);
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const handler = (newBid: Bid) => {
      ...
      setHighestBidderId(String(newBid.user_id));
      toast.success(
        `New bid: ${new Intl.NumberFormat("en-IN", {
          style: "currency",
          currency: "INR",
          maximumFractionDigits: 0,
        }).format(newBid.amount)} by ${newBid.username}`,
  ...
  useEffect(() => {
    ...
    const banhandler = ({
      ...
      if (user?._id === userId) {
        setUser((prev) => (prev ? { ...prev, banned } : prev));
        if (banned) toast.error("You have been banned from bidding.");
        else toast.success("Your ban has been lifted.");
      }
    };

    socket.on("change-ban-status", banhandler);

    return () => {
      socket.off("change-ban-status", banhandler);
    };
  }, [socket, user?._id]);

  const isCurrentHighestBidder =
    !!user && !!highestBidderId && String(highestBidderId) === String(user._id);

  const isAuctionActive = auction?.auction_status === "ACTIVE" && !auctionEnded;
  const isWinner = !!user && winner?.winnerId === String(user._id);
  ...
  const handlePlaceBid = useCallback(async () => {
    ...
    const minBid = (highestBid ?? auction.current_highest_bid ?? 0) + 1;
    if (bidValue < minBid) {
          ...
          <div className="flex flex-col gap-5">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-red-500">
                    {formatINR(highestBid ?? auction.current_highest_bid ?? 0)}
                  </p>
                </div>
            ...
            {auctionEnded && winner && (
              ...
              >
                ...
                >
                  <Trophy
                    className={`w-6 h-6 ${isWinner ? "text-yellow-500" : "text-gray-400"}`}
                  />
                </div>
                <div>
                  <p
                ...
                {!isAuctionActive ? (
                  <div className="flex flex-col items-center gap-2 py-4 px-3 bg-orange-50 rounded-xl border border-orange-100">
                    ...
                    <p className="text-xs text-orange-500 text-center">
                      Bidding is temporarily disabled. Please wait.
                    </p>
                  </div>
                ) : isCurrentHighestBidder ? (
